...
void NetworkInterface::send_datagram( const InternetDatagram& dgram, const Address& next_hop )
{
  ...
  if ( it == arp_table_.end() ) {
    ...
    ARPMessage arp = ARPMessage( ethernet_address_, 
                                 ip_address_.ipv4_numeric(), 
                                 {}, 
                                 next_hop_ipv4_numeric_ );
    ...
    if ( frame.header.type == EthernetHeader::TYPE_ARP) {
      ...
      if ( parse( arp, frame.payload ) ) {
        if ( arp.opcode == ARPMessage::OPCODE_REQUEST && arp.target_ip_address == ip_address_.ipv4_numeric() ) {
            ARPMessage reply = ARPMessage( ethernet_address_, 
                                           ip_address_.ipv4_numeric(), 
                                           arp.sender_ethernet_address, 
                                           arp.sender_ip_address );
            reply.opcode = ARPMessage::OPCODE_REPLY;
            EthernetFrame reply_frame = EthernetFrame( EthernetHeader( arp.sender_ethernet_address, ethernet_address_, EthernetHeader::TYPE_ARP ),
                                                       serialize( reply ) );
        ...
        for ( auto it = datagrams_to_send_.begin(); it != datagrams_to_send_.end(); ) {
          ...
          if ( arp_table_.find( next_hop.ipv4_numeric() ) != arp_table_.end() ) {
            EthernetFrame send_frame = EthernetFrame( EthernetHeader( next_hop_ethernet_address, ethernet_address_, EthernetHeader::TYPE_IPv4 ),
                                                      serialize( dgram ) );
            transmit( send_frame );
